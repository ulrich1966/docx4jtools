...
public class HeaderTest3 {
	...
	public static void main(String[] args) throws Exception {
		...
		FooterPart cover_ftr_part = new FooterPart(new PartName("/word/cover-footer.xml")), content_ftr_part = new FooterPart(new PartName("/word/content-footer.xml"));

		pkg.getParts().put(cover_ftr_part);
		pkg.getParts().put(content_ftr_part);

		// Ftr cover_ftr = factory.createFtr(), content_ftr =
		// factory.createFtr();
		// page number test
		...
		Relationship content_ftr_rel = main_part.addTargetPart(content_ftr_part);

		cover_ftr.getContent().add(makeParagraph(cover_ftr_part, "Cover footer text"));
		content_ftr.getContent().add(makeParagraph(content_hdr_part, "Content footer text"));
